...
features_list=st.sidebar.multiselect("select the feature to create histogram", ('RI', 'Na', 'Mg', 'Al', 'Si', 'K', 'Ca', 'Ba', 'Fe'))
st.set_option('deprecation.showPyplotGlobalUse', False)
for i in features_list:
...
features_list=st.sidebar.multiselect("select the feature to create boxplot", ('RI', 'Na', 'Mg', 'Al', 'Si', 'K', 'Ca', 'Ba', 'Fe'))
st.set_option('deprecation.showPyplotGlobalUse', False)
for i in features_list:
  st.subheader(f'boxplot for {i}')
  plt.figure(figsize=(16,9))
  sns.boxplot(glass_df[i], color='green', orient='h')
  st.pyplot()